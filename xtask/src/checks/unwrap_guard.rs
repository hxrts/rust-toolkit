...
pub fn run(repo_root: &Path, config: &ToolkitConfig) -> Result<FlatFindingSet> {
    ...
    for path in collect_rust_policy_files(
        ...
        for matched in banned_re.find_iter(&masked) {
            ...
            if !has_exemption {
                let call = matched.as_str().trim_start_matches('.');
                let call = call.split('(').next().unwrap_or(call).trim();
                findings.entries.insert(format!(
                    "{rel}:{line_no}: `{call}` requires a preceding `{}` rationale \
                     comment",
